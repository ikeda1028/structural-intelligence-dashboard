...
import {validateInput,parseResearch} from './core.mjs';
...
export async function getPublicData(_request,{env=process.env}={}) {
 const s=settings(env);
 return json({...seed,municipalities:registry,registryAvailable:true,registryError:'',masterSource:MASTER,masterFetchedAt:'2026-09-26',registryMode:'bundled-snapshot',release:'2026-09-26-v0.2',researchConfigured:Boolean(s.enabled&&s.key&&s.db&&s.service),notice:'全国名称台帳のスナップショットです。詳細資料は3自治体9件、統計値は未取得。AI結果は事実認定・紹介承諾ではありません。'});
}
