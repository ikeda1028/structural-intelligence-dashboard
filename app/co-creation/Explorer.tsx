...
export default function Explorer() {
  const [data,setData]=useState<RecordData|null>(null),[error,setError]=useState(''),[notice,setNotice]=useState('');
  ...
  const [pref,setPref]=useState(''),[query,setQuery]=useState(''),[page,setPage]=useState(0),[saved,setSaved]=useState<RecordData[]>([]);
  useEffect(()=>{
    const controller=new AbortController();
    fetch('/api/co-creation',{signal:controller.signal,cache:'no-store'}).then(async r=>{if(!r.ok)throw Error('公開データを取得できません。再読込してください。');return r.json();}).then(setData).catch(e=>{if(e.name!=='AbortError')setError(e.message);});
  ...
  return <main className={styles.root}>
    ...
    {data&&tab==='municipalities'&&<section className={styles.panel}><p className={styles.eyebrow}>NATIONWIDE REGISTRY</p><h2>自治体台帳</h2><div className={styles.search}><label>都道府県<select value={pref} onChange={e=>{setPref(e.target.value);setPage(0);}}><option value="">すべて</option>{prefectures.map(([code,label])=><option key={code} value={code}>{label}</option>)}</select></label><label className={styles.grow}>自治体名・コード<input value={query} onChange={e=>{setQuery(e.target.value);setPage(0);}} placeholder="例：三浦市、うるま市、14210"/></label></div><p className={styles.small}>{municipalities.length.toLocaleString()}件。資料未収録は「課題なし」ではありません。政令市の行政区は市全体との重複を避けて除外し、東京23区を含みます。</p><div className={styles.tableWrap}><table><thead><tr><th>コード</th><th>都道府県</th><th>自治体</th><th>資料の収録</th></tr></thead><tbody>{municipalities.slice(page*50,page*50+50).map((m:RecordData)=>{const n=data.evidence.filter((e:RecordData)=>e.municipality_code===m.code).length;return <tr key={m.code}><td>{m.code}</td><td>{m.prefecture}</td><td>{m.name}</td><td>{n?`${n}件 · 全文調査ではありません`:'未収録'}</td></tr>;})}</tbody></table></div><div className={styles.pagination}><button disabled={page===0} onClick={()=>setPage(page-1)}>前へ</button><span>{page+1} / {Math.max(1,Math.ceil(municipalities.length/50))}</span><button disabled={(page+1)*50>=municipalities.length} onClick={()=>setPage(page+1)}>次へ</button></div><SourceLink source={sources.get('gsi-master')}/><p className={styles.small}>元データの最終変更：2024年1月9日。2026年の最新台帳との完全突合は未実施。取得日時：{data.masterFetchedAt||'未取得'}。</p></section>}
