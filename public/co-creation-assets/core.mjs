/** Pure functions shared by public browser and server. No hidden contacts or scores. */
...
export function parseResearch(response, seed, input) {
  ...
  if(!data || typeof data !== 'object') throw Error('調査結果がありません');
  const official = safeUrl(data.official_url);
  if (data.ambiguous !== false || !official || !allowed.has(official)) throw Error('同名法人または公式サイトを特定できません。公式URLを指定して再調査してください');
